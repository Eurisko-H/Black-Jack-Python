...
class DBSetup:
    ...
    def setup_db(self, player):
        ...
        if money >= 0:
            self.cursor.execute(
                "UPDATE game SET win_money = win_money + ? WHERE name = ?",
                (abs(money), name),
            )
        else:
            self.cursor.execute(
                "UPDATE game SET lost_money = lost_money + ? WHERE name = ?",
                (abs(money), name),
            )
    ...
    def get_all_user(self):
        try:
            query = self.conn.execute("SELECT * FROM game ORDER BY name ASC;")
